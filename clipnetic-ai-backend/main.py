...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, framerate=25):
    ...
    for t_index, track in enumerate(tracks):
        ...
        for f_index, frame in enumerate(track["track"]["frame"].tolist()):
            slice_start = max(f_index - 30, 0)
            slice_end = min(f_index + 30, len(score_array))
            score_slice = score_array[slice_start:slice_end]
            avg_score = float(np.mean(score_slice)
                              if len(score_slice) > 0 else 0)

            faces[frame].append({'track': t_index, 'score': avg_score,
                                 's': track['proc_track']['s'][f_index],
                                 'x': track['proc_track']['x'][f_index],
                                 'y': track['proc_track']['y'][f_index]})
    ...
    for f_index, fname in tqdm(enumerate(fList), total=len(fList), desc="Creating vertical video"):
        ...
        if mode == "resize":
            ...
            crop_x = (bg_width - target_height) // 2
